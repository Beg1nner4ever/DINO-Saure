...
\emph{ViT-Tiny} (patch 8 ; \emph{backbone} $\sim$5,5\,M paramètres, $\sim$11\,M avec la tête de
projection) sur le jeu de données STL-10, évaluons la
qualité des représentations gelées par un classifieur $k$-NN, et comparons à deux lignes de base
de même architecture (initialisation aléatoire et entraînement supervisé). Nous retrouvons
partiellement la propriété émergente phare du papier --- des cartes d'auto-attention qui se
structurent autour des objets sans supervision (quoique bruitées à notre échelle) --- et menons trois ablations inspirées de la Table~7 du
papier (encodeur à momentum, multi-crop, température de l'enseignant). Comparées \emph{à durée égale}
aux résultats du papier, l'effondrement par retrait du momentum se reproduit nettement, tandis que
les effets du multi-crop et de la température, présents et de bon signe, restent atténués par notre
budget réduit. Notre but n'est pas
...
réaliste sur Colab. Les alternatives envisagées (MAE~\cite{he2022mae}, Barlow
Twins~\cite{zbontar2021barlow}, DDPM~\cite{ho2020ddpm}) ont été écartées
...
supervisée fournit la comparaison \guillemotleft\ équitable\,\guillemotright{} (même backbone, même protocole $k$-NN). On
...
bloc). On observe une attention spatialement structurée --- certaines têtes se concentrent plutôt
sur l'objet que sur le fond ---, quoique nettement plus bruitée qu'avec les grands modèles du papier
(Figures~1 et~3). Cette structuration, même imparfaite avec un ViT-Tiny peu entraîné, va dans le
sens de la propriété émergente de DINO ; nous l'interprétons comme un indice qualitatif encourageant,
pas comme une segmentation propre.
...
(concentrée sur l'objet) même avec un ViT-Tiny peu entraîné, indice qualitatif --- quoique bruité
--- de la propriété émergente (Figure~\ref{fig:attn}).
