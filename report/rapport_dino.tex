...
segmentent les objets sans supervision --- et menons trois ablations inspirées de la Table~7 du
papier (encodeur à momentum, multi-crop, température de l'enseignant). Notre but n'est pas
...
\midrule
Jeu de données        & ImageNet-1k (1,28\,M) & STL-10 (sous-échant. 20\,000) \\
Backbone              & ViT-S/16, ViT-B/8     & ViT-Tiny/8 ($\sim$5--6\,M) \\
Résolution            & 224                   & 96 (global) / 48 (local) \\
Époques               & 300                   & 40 (principal), 20 (ablations) \\
...
\midrule
Initialisation aléatoire (borne basse)        & \TODO{random\_knn} \\
Supervisé (5\,000 images étiquetées)          & \TODO{supervised\_knn} \\
\textbf{DINO (auto-supervisé, 20\,000 img.)}  & \TODO{dino\_knn} \\
\midrule
\emph{Pour mémoire} : supervisé, précision de classification directe & \TODO{supervised\_test\_cls} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[h]
\centering
% \includegraphics[width=0.6\linewidth]{../figures/loss_curve.png}
\caption{Courbe de perte du pré-entraînement DINO (\texttt{figures/loss\_curve.png}).
Une décroissance régulière sans plateau précoce à valeur dégénérée indique l'absence
d'effondrement. \emph{Insérer la figure après le run.}}
\label{fig:loss}
\end{figure}
...
\begin{figure}[h]
\centering
% \includegraphics[width=0.9\linewidth]{../figures/attention_cat.png}
\caption{Cartes d'attention par tête sur des images de test
(\texttt{figures/attention\_*.png}). \emph{Insérer 1--2 exemples après le run.}}
\label{fig:attn}
\end{figure}
...
\emph{Résultat} : \TODO{A1\_no\_momentum} \% --- \emph{interpréter : effondrement observé ?
comparer à la borne aléatoire du Tableau~\ref{tab:main}}.
...
de $\sim$3--5 points. \emph{Résultat} : \TODO{A2\_no\_multicrop} \% --- \emph{quantifier l'écart
avec DINO complet}.
...
centrage/affûtage. \emph{Résultats} : $\tau_t{=}0{,}04$ : \TODO{A3\_tau\_t=0.04}\,\% ;
$\tau_t{=}0{,}07$ : \TODO{A3\_tau\_t=0.07}\,\% ; $\tau_t{=}0{,}10$ : \TODO{A3\_tau\_t=0.1}\,\%.
...
\midrule
--- & DINO complet (référence)            & \TODO{dino\_knn} \\
A1  & Sans momentum (copie directe)       & \TODO{A1\_no\_momentum} \\
A2  & Sans multi-crop (2 globales)        & \TODO{A2\_no\_multicrop} \\
A3  & $\tau_t = 0{,}04$                    & \TODO{A3\_tau\_t=0.04} \\
A3  & $\tau_t = 0{,}07$                    & \TODO{A3\_tau\_t=0.07} \\
A3  & $\tau_t = 0{,}10$                    & \TODO{A3\_tau\_t=0.1} \\
\bottomrule
\end{tabular}
\end{table}
...
étiquetées utilisées (\texttt{figures/adaptive\_curve.png}). Le plateau indique la taille de
données suffisante ; \emph{insérer après le run}. Arrêt à \TODO{images} images /
\TODO{epoch} époques, meilleur k-NN sonde \TODO{best\_probe}\,\%.}
\label{fig:adapt}
\end{figure}
...
\paragraph{Ce qui fonctionne.} \emph{À compléter avec les chiffres.} Typiquement : DINO dépasse
nettement la borne aléatoire, confirmant que le mécanisme apprend des représentations utiles à
échelle réduite ; l'ablation A1 devrait montrer un effondrement franc, validant le rôle
\emph{load-bearing} de l'enseignant EMA ; les cartes d'attention sémantiques émergent même avec un
ViT-Tiny peu entraîné.

\paragraph{Ce qui fonctionne mal ou reste incertain.} \emph{À compléter.} Pistes d'analyse
honnête : l'écart DINO vs. supervisé peut être faible, voire défavorable, car (i) 40 époques et
20\,000 images sont très en deçà du régime où le SSL brille, (ii) $K=2048$ et un ViT-Tiny limitent
la capacité, (iii) le $k$-NN à un seul $k$ et une seule graine présente une variance non mesurée.
Si une ablation ne reproduit pas la tendance attendue (p.\,ex. A2 sans baisse nette), il faut le
dire et l'expliquer (durée d'entraînement trop courte pour que le multi-crop paie, bruit
d'estimation).
