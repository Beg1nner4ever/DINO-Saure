...
\begin{itemize}[nosep]
  \item \textbf{Mémoire GPU.} La passe \emph{multi-crop} --- six vues par image (2 globales $96^2$
  + 4 locales $48^2$) traversant l'étudiant \emph{et} l'enseignant, avec une tête de dimension
  $K=2048$ --- sature la mémoire du T4 (16\,Go) et provoque des erreurs \emph{out-of-memory}.
  \item \textbf{Coupures de session.} Les sessions gratuites Colab s'interrompent (déconnexion sur
  inactivité, limite de durée), ce qui est incompatible avec un entraînement de plusieurs heures.
\end{itemize}
...
A1/A2) a tourné sans interruption en $\sim$6\,h sur le GPU intégré, là où l'environnement T4 ne
permettait pas de le mener à terme. Cette dualité résume un arbitrage fréquent en pratique :
...
passe sous un seuil. La quantité de données « suffisante » devient ainsi empirique plutôt
qu'arbitraire.}
\label{fig:loop}
\end{figure}
