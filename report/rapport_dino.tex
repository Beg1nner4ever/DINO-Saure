...
($48\times48$). L'enseignant --- moyenne mobile exponentielle (EMA) de l'étudiant, avec
\emph{stop-grad} --- ne traite que les vues globales ; l'étudiant traite toutes les vues. Chaque
...
d'une cible stable et légèrement \guillemotleft\ en avance\,\guillemotright{}, analogue à un ensemble de modèles (Polyak
averaging).
...
\begin{itemize}[nosep]
  ...
  (16\,Go). On la contourne en divisant le batch par deux ($256\to128$) et en ajustant le LR par la
  règle de mise à l'échelle linéaire, au prix d'un réglage dégradé.
...
\begin{itemize}[nosep]
  ...
  interpolation) n'est pas implémenté sur MPS et bascule sur CPU (léger surcoût) ; \texttt{pin\_memory}
  et la précision mixte (AMP) sont également indisponibles, d'où un entraînement en \texttt{float32}.
\end{itemize}
...
étiquetées utilisées ; la bande grisée indique le bruit d'échantillonnage de la sonde ($\pm1{,}5$ pt
sur 1\,000 requêtes). La précision croît de 23,7\,\% (4\,000
